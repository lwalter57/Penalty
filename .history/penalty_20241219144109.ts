...
const updateScore = (score: Score, teamAAcu: number, teamBChange: number): Score => ({
  teamA: score.teamA + teamAAcu,
...
const displayHistory = (history: HistoryEntry[]): void => {
  history.forEach((entry) => {
    console.log(
      `Tir ${entry.shotNumber} : Score : ${entry.score} (Équipe A : ${entry.teamAAcu} | Équipe B : ${entry.teamBChange})`
    );
  });
};
