...
const penaltyShootout = (history: HistoryEntry[] = [], score: Score = { teamA: 0, teamB: 0 }, shotNumber: number = 1): void => {
  ...
  // Vérification de la condition de victoire par 3 buts d'écart
  if (Math.abs(newScore.teamA - newScore.teamB) >= 3) {
    console.log(
      `Victoire : ${newScore.teamA > newScore.teamB ? "Équipe A" : "Équipe B"} (Score : ${newScore.teamA}/${newScore.teamB}) avec 3 buts d'écart.`
    );
    return;
  }

  // Prolongation ou victoire après 5 tirs
