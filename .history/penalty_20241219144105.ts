...
const updateScore = (score: Score, team: number, teamBChange: number): Score => ({
  teamA: score.teamA + team,
...
const displayHistory = (history: HistoryEntry[]): void => {
  history.forEach((entry) => {
    console.log(
      `Tir ${entry.shotNumber} : Score : ${entry.score} (Équipe A : ${entry.team} | Équipe B : ${entry.teamBChange})`
    );
  });
};
