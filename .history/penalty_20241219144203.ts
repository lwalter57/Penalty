...
const updateScore = (score: Score, teamActualScore: number, teamBChange: number): Score => ({
...
const displayHistory = (history: HistoryEntry[]): void => {
  history.forEach((entry) => {
    console.log(
      `Tir ${entry.shotNumber} : Score : ${entry.score} (Équipe A : ${entry.teamActualScore} | Équipe B : ${entry.teamBChange})`
    );
  });
};
