...
const updateScore = (score: Score, teamA: number, teamBChange: number): Score => ({
  teamA: score.teamA + teamA,
...
const displayHistory = (history: HistoryEntry[]): void => {
  history.forEach((entry) => {
    console.log(
      `Tir ${entry.shotNumber} : Score : ${entry.score} (Équipe A : ${entry.teamA} | Équipe B : ${entry.teamBChange})`
    );
  });
};
